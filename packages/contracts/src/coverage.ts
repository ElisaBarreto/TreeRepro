 ...
 * one definition: `percentWithData = withData / cells`, `percentValidated =
 * validated / cells` (validated = a species × trait holding a record with a
 * `confirm` annotation and no `withdraw`), both integers 0–100 rounded half
 * up (`percentHalfUp`), and `0` for an empty grid.
 * @rfc RFC-69 R5
 */
 ...
 * lacking data (`mode=missing`, the default) or the lowest validated share
 * (`mode=least_validated`), over the full unfiltered visible grid.
 * @rfc RFC-69 R7
 */
export const coverageTopQuerySchema = z.strictObject({
  mode: z.enum(['missing', 'least_validated']).optional(),
