...
**Fix:** `Branch-Protection` is the only one with a file behind it: set the four options in `infra/github/ruleset-main.json`, add `.github/CODEOWNERS`, and have the owner run `scripts/github-admin.sh`; `Code-Review` then follows once PRs are approved by the other person. `Maintained` needs time and `CII-Best-Practices` needs the registration. For a finding the project chooses not to act on, dismiss the alert as "won't fix" with that reason; a dismissed Scorecard alert stays dismissed on later runs. `Pinned-Dependencies` and `Fuzzing` are code-level and are satisfied (corepack pins pnpm by hash, `fast-check` property tests in `apps/api`).
