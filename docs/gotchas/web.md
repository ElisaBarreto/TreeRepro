...
## An `api/*.test.ts` fails with `ApiError: Unexpected response from the server`
**Symptom:** A fetcher test that mocks `fetch` rejects with `RESPONSE_INVALID`, `cause` a `ZodError` naming paths such as `data.0.traitCount`, although the page test using the same fixture passes.
**Cause:** `apiFetch` parses every successful body with the contracts schema the fetcher names (RFC-13 R1, issue #116), so a mocked body that is not what the API sends — a trimmed object, a field the schema omits such as `runBy` on a health import, `{ status: 'ok', user }` where the API answers `{ user }` — fails there. A page test never reaches `apiFetch` (it mocks the fetcher module), which is why the same fixture passes it; the type system did not catch it either, because an identifier is not a fresh object literal and the excess-property check never fires.
**Fix:** Mock the API's shape, not the page's needs: reuse the typed fixtures of `apps/web/src/test/*-fixtures.ts` (`ADMIN_USER`, `SPECIES`, `GENERA`, …) and spread over them, and answer a request that returns nothing with `{ data: { status: 'ok' } }` (`okStatusSchema`) — the API never sends 204. When the fixture and the schema disagree, read the route (`apps/api/src/http/routes/`) and its RFC before touching either: `POST /api/auth/password/forgot` really does answer `{ status: 'sent' }` (RFC-21 R5), and the fix there was a schema of its own, not a fixture edit.

## A page scrolls sideways at phone width although its table scrolls on its own
**Symptom:** At 390px the document is wider than the viewport (`scrollWidth` > 390) on pages whose wide content sits in `Table`'s `overflow-x-auto` wrapper or in a grid that only gets columns from `md` up (issue #160).
**Cause:** Two layout rules. An absolutely positioned descendant with no positioned ancestor (a `sr-only` column header) takes the viewport as its containing block, so it escapes the scroll container and extends the page. And a `grid` without `grid-cols-*` below its breakpoint has one implicit `auto` track, which grows to the widest item's min-content (a table, a long option in a `<select>`), not to the container.
**Fix:** `Table`'s wrapper is `relative`; a responsive grid starts from `grid-cols-1` (`minmax(0, 1fr)`) — `Section`, `TaxaPage`, `PendingPage`. Check with `document.documentElement.scrollWidth` at 390px (`apps/e2e/tests/mobile.spec.ts` does on `/app`).
