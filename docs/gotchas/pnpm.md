...
**Fix:** Wait for the version to age, or pick the previous one. For an urgent security patch add `name@exact.version` to `minimumReleaseAgeExclude` with a comment saying when it can be removed. pnpm never prunes by age: an entry stays until you delete it or the lockfile stops resolving that exact version. The list added on 2026-09-12 (versions already locked before the guard existed) must be deleted by hand after 2026-09-18.
