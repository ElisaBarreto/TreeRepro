...
**Cause:** `apps/api` has no `dom` lib (the API is a server process; it never runs in a browser), so `@types/node` types `Body#json()` — and therefore `Response#json()` / `Request#json()` — as `Promise<unknown>` instead of `lib.dom.d.ts`'s `Promise<any>`.
