...
**Date:** 2026-09-17
**Status:** 11a implemented (API and web); 11b implemented (API and web); plan `-11c-coverage.md` not yet started
...
| Topic | Decision |
|---|---|
| Contributions | `GET /api/me/contributions` (records or annotations of the viewer) and `GET /api/admin/users/:id/contributions` (`contributions.read`, manager+; under the admin prefix like every other per-user route, RFC-50). One page `/app/contributions`, reused for another user with `?userId=`. |
...
(Wording: the empty state and the "missing" numbers read "no record yet" — coverage counts withdrawn records too, RFC-69 R2.)
