...
| `dataset/records.ts` | Detail gains `supersedes` / `supersededBy`; `reviewStatusSql` reused by the queues (and by `curation.ts`). A bare `Column` argument (`reviewStatusSql(traitRecords.id)`) only qualifies correctly inside a multi-table select such as `itemQuery`'s joined query; a caller selecting from `trait_records` alone (`curation.ts`'s `annotateRecord`, `setAccepted`) must instead pass `` reviewStatusSql(sql`${traitRecords.id}`) `` — Drizzle renders a bare column unqualified in a single-table select, and the function's own correlated subquery over `record_annotations` then has its `record_annotations.id` shadow that unqualified name (`docs/gotchas/dataset.md`). |
