...
| Lookup | GBIF species match API only (`https://api.gbif.org/v1/species/match?name=&datasetKey=`), two calls per proposal (backbone, WCVP checklist), fixed host, `redirect: 'manual'`, 5 s timeout, injected client (RFC-80 pattern), best effort: a failed lookup stores `null` and never blocks the proposal. |
...
- **R5** One e-mail per recipient (`digestEmail` template: plain text and HTML, subject `TreeRepro digest — <date>`, the counts, the two lists with links to the record drawer URLs, the queue sizes) sent through the mailer of RFC-20 with the 5 s timeout; a failed send is logged and counted in `detail.failed`; the run completes when every send was attempted. Audit `digest.sent` with `metadata: { recipients, failed, windowStart, windowEnd }` (no addresses).
...
- **R1** Client `ctx.taxonomy` (`TaxonomyClient.match(name)`) calls `GET https://api.gbif.org/v1/species/match?verbose=true&name=<encoded>` (backbone) and the same with `datasetKey=<WCVP_GBIF_DATASET_KEY>` (the WCVP checklist hosted by GBIF; key from configuration, the plan records the current one and the check that it lists WCVP). Transport as RFC-80 R3 (fixed host, `redirect: 'manual'`, 5 s, 1 MiB cap, no retry); a fake client in tests.
- **R2** Mapping of one GBIF answer: `{ matchType: 'EXACT' | 'FUZZY' | 'HIGHERRANK' | 'NONE', confidence, usageKey, scientificName, canonicalName, rank, status, family, genus, acceptedUsageKey?, note? }`; `HIGHERRANK` and `NONE` are reported as `none` with the higher rank named.
...
- **R1** `GET /api/admin/health` (`health.read`) answers `{ users: { active, invited, suspended, signedInLast7d, signedInLast30d }, dataset: { species, activeSpecies, traits, activeTraits, references, records, coverageCells, acceptedCells }, activity: { records7d, annotations7d, proposals7d, byDay: [{ day, records, annotations }] (14 days) }, queues: { pendingGroups, disputed, contested, proposals }, jobs: { auditPurge: <run>, digest: <run> } (the newest `job_runs` row per kind as `{ startedAt, finishedAt, status, detail, error } | null`), imports: [<RFC-64 R11 item> … 5 newest], computedAt }`. `signedInLast7d` and `signedInLast30d` count distinct `actor_user_id` over the audit log's `auth.login.success` entries in the window (one indexed query; the audit log exists for this). Cached one minute in Redis (`health`).
- **R2** No PII: counts, ids and timestamps only. Audit `admin.accessed` is not repeated (already emitted by the admin layout).
