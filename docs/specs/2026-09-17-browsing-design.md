...
| Search tiers | Tier 1: canonical name equals `q` (case-insensitive) → only that species. Tier 2: canonical substring. Tier 3: alternative-name substring. The first non-empty tier is the result; the cursor carries the tier. `matchedName` + `matchedNameType` say how a row matched. |
...
- **R6 amended — search tiers.** With `q`: tier 1 keeps the species whose `lower(canonical_name) = lower(q)`; if none, tier 2 keeps canonical substring matches; if none, tier 3 keeps alternative-name substring matches (any type). The non-empty tier with the lowest number is answered, ordered as today; the cursor encodes the tier so a page never mixes tiers. `matchedName` (alternative name that matched, else null) gains `matchedNameType: 'gbif' | 'synonym' | 'common' | null`. Without `q` nothing changes.
...
- **R5 amended.** `GET /api/traits?categoryKey=&valueType=&q=` filters the dictionary server-side (the page still loads it whole for selects; filtering is for deep links). The trait entry gains `speciesCount` (visible species with a coverage row) — read from `species_trait_coverage` grouped by trait, one query, cheap.
- **R7 (new).** `GET /api/traits/:id` (`dataset.read`, visibility): the trait entry plus `category: { key, label }`, `speciesWithData`, `speciesMissing` (visible active species without a coverage row), `acceptedCount` (species whose current accepted value is on this trait), and `distribution`: for a categorical trait `levels: [{ level: { id, key }, speciesCount, recordCount }]` (harmonised records grouped by level, species counted distinct); for a quantitative trait `numeric: { min, median, max, speciesCount }` over harmonised records. Unknown or invisible → 404 `TRAIT_NOT_FOUND`. The distribution query runs over `trait_records` for one trait through `trait_records_trait_idx`; it is bounded by the largest trait (~1–2% of the records) and cached in Redis for 10 minutes (`trait:<id>:distribution`), invalidated by nothing — a ten-minute lag on a summary is acceptable and the counts are monotonic.
...
Species merges; a map view; ordering species by "records awaiting validation" (dashboard covers it per user); per-level counts on the species list; a references export.
