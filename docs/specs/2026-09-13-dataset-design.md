...
| Provenance | Every record carries its origin (`import` with batch and row number, or `manual` with the creating user), its raw source columns, and `created_at`. Curation steps (plan 06) are append-only rows that reference the record; the current stance of a scientist is their latest annotation; the current accepted value is the latest decision. Nothing is ever overwritten. |
...
| Duplicates | A unique index over the claim (nulls not distinct) makes an identical claim impossible to store twice; the import counts the collisions as duplicates; a scientist trying to add an existing claim is told to confirm it instead (plan 06). |
...
| `unknown_levels jsonb null` | top 30 `{ trait, value, count }` for the report and the UI |
