...
One decision changes the foundation design: **users are never deleted or anonymized by the system.** The people in this system are research collaborators whose contributions are cited in scientific publications. Offboarding is suspension.

The GDPR still applies (collaborators are natural persons). Lawful basis: Article 6(1)(f) — the legitimate interest of the project and of the scientific community in the integrity and attributability of the scientific record. Necessity: a collaborator's name and email are needed for as long as their contributions can be published, cited or audited; erasing an identity would make attribution impossible or seriously impair it, so the Article 17(3)(d) exemption applies to that extent. Article 89(1) requires safeguards for this kind of processing, and the system already has them: encryption at rest (RFC-40), access limited to permission-guarded routes (RFC-32), the append-only audit log (RFC-41) and its retention (RFC-42), and suspension instead of deletion. Scope and review: the system offers no erase endpoint; an erasure request from a person with no attributable contributions is assessed by the project owner (the data controller) outside the API, and retained identities are reviewed whenever a collaborator is suspended.
