...
- **R7** Every request resolves the session before routing: cookie → record → user row. When the record is missing or expired, or the user is not `active`, the session is deleted, the cookie cleared and the request continues unauthenticated. Otherwise `user` and `session` are available to handlers. A Bearer API key is resolved first (RFC-82 R3, R4).
- **R8** `requireSession` answers 401 `AUTH_UNAUTHENTICATED` when no cookie session was resolved; a request authenticated by an API key has no session (RFC-82 R6).
- **R9** `POST /api/auth/logout` deletes the current session and clears the cookie (audit `auth.logout`). `POST /api/auth/logout-all` deletes every session of the user including the current one and revokes every active API key of the user (RFC-82 R3) (audit `auth.logout_all`). Both answer `{ data: { status: "ok" } }`.
...
- **R12** The client IP is the last entry of `X-Forwarded-For`; without the header it is `unknown`. Caddy sets that header to a single value: `CF-Connecting-IP` when the peer is in Cloudflare's published ranges (production runs behind the Cloudflare proxy), otherwise the remote address, discarding any incoming value. It is used for the audit log and rate limiting only and never logged (RFC-02 R7).
...
- 2026-09-25 — R12: client IP behind the Cloudflare proxy (issue #169).
- 2026-09-26 — R7, R8: a Bearer API key is resolved first and carries no session (RFC-82, plan 14a).
- 2026-09-26 — R9: sign out everywhere also revokes every active API key of the user (RFC-82 R3).
