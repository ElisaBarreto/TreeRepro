...
- **R7** `POST /api/auth/totp/disable { password, code | recoveryCode }` (session): the password must verify (401 `AUTH_INVALID_CREDENTIALS`) and the code must be a valid TOTP or unused recovery code (401 `AUTH_TOTP_INVALID`); then `totp_secret`, `totp_enabled_at` and all recovery codes are cleared and every active API key of the user is revoked (RFC-82 R3); audit `auth.totp.disabled`. 409 `AUTH_TOTP_NOT_ENABLED` when TOTP is off.
...
- 2026-09-12 — created.
- 2026-09-12 — accepted.
- 2026-09-26 — R7: disabling TOTP also revokes every active API key of the user (RFC-82 R3).
