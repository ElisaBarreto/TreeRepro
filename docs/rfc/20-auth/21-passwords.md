...
- **R4** Login always runs an argon2 verification: against the stored hash when the user is `active` or `suspended`, otherwise (an `invited` user, or no user for the given email) against a process-wide dummy hash generated at first use. Unknown email, wrong password and `invited` all answer 401 `AUTH_INVALID_CREDENTIALS` with the same body.
...
- **R6** `POST /api/auth/password/reset { token, newPassword }`: policy (R2, R3) first, then the token is consumed, the hash replaced and every session and every active API key of the user revoked (RFC-82 R3); no session is created. Audit `auth.password.reset`. An invalid token answers 400 `AUTH_TOKEN_INVALID`.
- **R7** `POST /api/auth/password/change { currentPassword, newPassword }` (session required): the current password must verify (401 `AUTH_INVALID_CREDENTIALS` otherwise), the policy applies to the new one, the hash is replaced, every session except the current one is revoked and every active API key is revoked (RFC-82 R3). Audit `auth.password.changed`.
...
- 2026-09-12 — created.
- 2026-09-12 — accepted.
- 2026-09-12 — R4: name only the statuses that exist (`invited`, `active`, `suspended`); drop the retired `deleted` status.
- 2026-09-26 — R6, R7: reset and change also revoke every active API key of the user (RFC-82 R3).
