...
- **R11** A `?` help tip is a button that toggles a `role="tooltip"` element it controls; it opens on click, focus and hover, closes on Escape and outside click; its text is plain content, never HTML from the API.
