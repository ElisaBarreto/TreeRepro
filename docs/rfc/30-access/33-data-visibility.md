...
- **R5** Writes: `POST /api/records` and `POST /api/records/pending/map` on an invisible species, trait or level answer the same 404 / 400 as for an unknown one; `POST /api/records/:id/annotations` on an invisible record answers 404. Catalog writes (`taxa.manage`, `traits.manage`, `references.manage`) take the same visibility; their holders are unrestricted by role design and the services do not assume it. `POST /api/records/pending/map` applies visibility to the trait and the levels; the pending rows it maps are not filtered by species — the queue routes require `records.review`, whose holders are unrestricted by role design (RFC-31 R10).
