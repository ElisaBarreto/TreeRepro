...
- **R5** Guard classes. Every registered route is exactly one of: public (RFC-22 R1), self-service — behind `requireSession` only —, or permission-guarded — behind `requirePermission`. The self-service list: `POST /api/auth/logout`, `POST /api/auth/logout-all`, `GET /api/auth/me`, `POST /api/auth/password/change`, `POST /api/auth/totp/setup`, `POST /api/auth/totp/confirm`, `POST /api/auth/totp/disable`, `GET /api/me/sessions`, `DELETE /api/me/sessions/:id`, `PATCH /api/me` (RFC-50 R11), `GET /api/help`, `GET /api/help/:slug` (RFC-73 R6). Every route that is neither public nor in the self-service list is permission-guarded, wherever its path lives (`/api/admin/*`, `/api/species`, …). A test enumerates the registered routes and fails on any route outside its class.
