...
- **R7** Anti-lockout: the last `active` user holding `admin` cannot lose it — `setUserRoles` answers 409 `ROLE_LAST_ADMIN` when the change would remove `admin` from that user. `assertNotLastAdmin(userId)` exposes the same check for suspension and erasure (RFC-5x); the check holds a transaction-scoped advisory lock so concurrent removals serialize. Suspended, invited and deleted holders do not count.
