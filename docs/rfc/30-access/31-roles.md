...
- **R10** Permission sets of the seeded roles. `contributor`: `dataset.read`, `records.create`, `records.annotate`, `taxa.propose`. `manager`: the contributor set plus `dataset.read_inactive`, `records.review`, `records.withdraw`, `imports.read`, `contributions.read`, `coverage.read`. Managers do not hold `traits.manage`: a missing level is escalated to the admin. No seeded role stores `records.withdraw_imported` (RFC-65 R4): only `admin` withdraws imported records (R15). No seeded role stores `help.edit` either: out of the box only `admin` edits the help pages (RFC-73 R7).
