...
- **R1** `GET /api/export/dataset.zip` requires `dataset.export`, which only the `admin` role holds (RFC-31 R15: no custom role may hold it). It replaces `GET /api/export/accepted.csv`.
- **R2** The archive holds two files, both over the viewer's visibility: the route resolves `visibilityOf(ctx, c)`, and every query carries `speciesVisible`, `traitVisible` and the record visibility of RFC-33 R2, so withdrawn records never appear (RFC-63 R13). `records.csv` has one row per visible record, pending ones included with their raw value. Its columns, in order: `record_code, family, genus, species, name_source, category, trait, unit, level, value_single, value_min, value_max, value_mean, value_sd, value_n, raw_value, references, origin, intent, responds_to, contested, n_validations, n_contests, created_at`. `annotations.csv` has one row per validation (`confirm`) on a visible record and one row per contest and per visible record it contests, with the columns `record_code, kind, user_name, date, reference, contest_record_code`; `kind` is `validation` or `contest`. In `records.csv`: `level` is the level key or empty; the value columns print as PostgreSQL prints `numeric`, or are empty; `references` is the record's references, primary first (RFC-63 R16), a reference named twice kept once in its earliest place, joined by `; `, each printed as its citation key or as `Personal observation` for a reference of kind `personal_observation`; `responds_to` is the responded record's `record_code`, or empty; `contested` is `true` or `false` (RFC-63 R14); `n_validations` and `n_contests` are RFC-63 R8's `validationCount` and `contestCount`, which count distinct users. In `annotations.csv`: a contest that is not withdrawn (resolved ones included, as `contestCount` counts them) gives one row for each visible record it contests (RFC-63 R14) — every visible record of each level a categorical contest names, or the one record a quantitative contest responds to; the row's `record_code` is that contested record, its `contest_record_code` is the `record_code`s of the records the contest created, in code order and joined by `; ` (empty for a contest that created none), `user_name` is the contest's author, `date` its time and `reference` empty; a validation row leaves `contest_record_code` empty; `reference` is the validation's supporting reference, printed as in `references` (its citation key, or `Personal observation`), or empty; `user_name` is the actor's name. User names only, never an e-mail address (RFC-02 R14, RFC-40). `created_at` and `date` are ISO 8601 UTC.
...
- **R5** The response streams: rows are read through a server-side cursor in batches and written into the archive as they arrive. The archive uses ZIP64, so `records.csv` may exceed 4 GiB. Neither a file nor the archive is ever held in memory. A download cancelled or aborted at any point, before its first chunk is read included, closes the cursor and returns its connection to the pool at once.
...
- 2026-09-26 — R2: a validation's `reference` prints as `references` does (citation key or `Personal observation`); `references` names a repeated reference once, in its earliest place, as the API's item does (plan 13i final review).
- 2026-09-26 — R5: a download cancelled before its first chunk is read also closes its cursor (issue #216: `POST /api/batch` cancels a ZIP answer unread, and each such cancel held a pooled connection for good).
