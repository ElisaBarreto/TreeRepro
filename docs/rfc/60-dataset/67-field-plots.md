...
- **R4** `GET /api/plots/:id/species?q=&cursor=&limit=` (`dataset.read`) lists the plot's species as species list items (RFC-60 R6, visibility applied); `GET /api/plots/:id/users?cursor=&limit=` (`plots.manage`) lists `{ id, name, status, restricted }` by name. The item carries no e-mail address: the address is `users.read` data wherever it appears (RFC-02 R14), and `plots.manage` does not imply `users.read`. A viewer who also holds `users.read` reaches the address through `GET /api/admin/users/:id` (RFC-50 R4); the SPA links the name to the user page only for such a viewer.
...
- 2026-09-20 — R4: the plot user item drops `email`; the address stays behind `users.read` (RFC-02 R14, issue #119 F-04).
