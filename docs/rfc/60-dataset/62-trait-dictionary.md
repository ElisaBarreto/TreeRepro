...
- **R5** `GET /api/traits?categoryKey=&valueType=&q=` (`dataset.read`) returns the whole dictionary: categories by `sort_order`, each with its traits by `key`, each with its levels by `sort_order` then `key`; the query parameters filter the dictionary server-side (the page still loads it whole for its selects — filtering answers deep links). For a restricted viewer (RFC-33 R1) inactive traits and inactive levels are omitted; for an unrestricted viewer they are included with `active: false`. Shape: `[{ key, label, traits: [{ id, key, valueType, unit, description, active, speciesCount, levels: [{ id, key, sortOrder, active }] }] }]` — `speciesCount` is the count of visible species with a `species_trait_coverage` row for the trait (`select trait_id, count(*) from species_trait_coverage c join species s … group by 1` is a scan of the coverage table, millions of rows), so it is cached 10 minutes per viewer class (`dictionary:species-counts:<u|r>`, `cachedJson`) rather than computed per request; a fresh number is not worth a scan per form select. Like R7's distribution cache, `speciesCount` is a global summary, never scoped to a plot-bound viewer's plots: such a viewer gets the restricted class (`r`) exactly like any other restricted viewer, and the count still includes species outside their plots — RFC-60 R6 makes the same choice for `traitCount`. The response itself is not cacheable beyond the request (the dictionary is editable, R6).
