...
- **R5** `coverageTotals(ctx, visibility)` answers `{ cells, withData, accepted, percentWithData, percentAccepted }` over every visible active species × visible active trait; cached 10 minutes under `coverage:totals:<u|r>`. Plan 11c later amends it to take filters.

## Open questions

Coverage metrics — the filtered per-category and per-trait breakdown and the "Top gaps" list — are R6–R7 of this RFC; they are specified in `docs/specs/2026-09-17-workspace-design.md` §5 and land with plan 11c, not here.
