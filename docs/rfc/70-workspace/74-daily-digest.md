...
- **R2** Schedule: `startDigestTimer` ticks every hour (first tick 60 s after start, `unref`); a tick runs the digest when no `digest` run with status `completed` or `skipped` finished in the last 23 h 30 min (so an hourly tick lands once a day and a restart never doubles it) **and** no `digest` run is still `running` from less than two tick intervals ago. The second condition covers the throw as the first covers the restart: a run that fails after its sends but before its `finished_at` is written never reaches a terminal status, so the last `completed` run keeps answering with its own, already-elapsed `finished_at` and its own `windowEnd`, and every following tick would recompute the identical window and mail every recipient again. A `running` run older than that guard postpones nothing, so a permanently stuck row delays the digest at most once and can never disable it. The window is `(last completed-or-skipped run's window end, now]`, or the last 24 h for the first run.
