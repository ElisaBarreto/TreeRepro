...
## Rules
- **R1** A DOI submitted by a client is first normalised: resolver prefixes (`https://doi.org/`, `http://doi.org/`, `https://dx.doi.org/`, `http://dx.doi.org/`, `doi:`) and surrounding whitespace are stripped; the result is lowercased. A DOI that does not match `^10\.\d{4,9}\/\S{1,200}$` after normalisation is malformed (400 `VALIDATION_FAILED`).
...
- **R5** In a transaction that links a source to a record or annotation, a DOI is resolved and inserted into `bibliographic_references` just-in-time: if `findReferenceByDoi(doi)` misses, the API calls the `DoiClient` and inserts a reference with `citationKey: doi:{doi}`, the resolved metadata, the actor as the creator, and an audit entry `source: 'doi'`; then it proceeds with the local id. `bibliographic_references` is unique on `lower(doi)`, so a DOI names at most one reference whatever the casing, and the insert is `ON CONFLICT DO NOTHING` followed by a re-read: a raised unique violation would abort the surrounding transaction and the re-read with it. Sources are distinct after resolution — naming a reference by its id and again by its DOI is one source, and the second is a 400 `VALIDATION_FAILED`.
