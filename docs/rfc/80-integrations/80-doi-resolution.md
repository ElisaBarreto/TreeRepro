...
- **R5** In a transaction that links a source to a record or annotation, a DOI is resolved and inserted into `bibliographic_references` just-in-time: if `findReferenceByDoi(doi)` misses, the API calls the `DoiClient` and inserts a reference with `citationKey: doi:{doi}`, the resolved metadata, the actor as the creator, and an audit entry `source: 'doi'`; then it proceeds with the local id.
