...
6. **Owner ruling 2026-09-25 — contests state the correct levels** (spec R-8 as amended; RFC-63 R14, RFC-70 R2, R3, R9, R10): 👎 on a level opens `AddEntriesDialog` with Contest chosen, that level unchecked and the other levels with records checked; for a categorical contest the "Responding to" select goes (it stays for a complement and a quantitative contest), and every categorical entry shows the required confirmation summary ("Validate red · Contest blue, yellow · Add green") before submit, sends the contested levels as `contestedLevelIds`, and on a 400 with path `contestedLevelIds` reloads the levels and asks for confirmation again. Concretely, Task 6 (`AddEntriesDialog`) drops the categorical "Responding to" choice, submits `contestedLevelIds` for a categorical contest, and keeps "Responding to" for complements and quantitative contests; its tests cover the summary, the confirmation and the reload after a 400 on `contestedLevelIds`. Where task bodies assume a contest responds to one record's level, the amended rules win.
