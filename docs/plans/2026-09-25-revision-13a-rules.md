...
9. **Owner ruling 2026-09-25 — contests state the correct levels** (spec R-8 as amended; RFC-63 R14, RFC-70 R2, R3, R9, R10): this branch carries the ruling into the spec (Amendments line, R-7–R-10, R-17, R-20, §2, §6) and the RFCs: RFC-63 Context, R2, R7, R8, R14; RFC-64 R15; RFC-65 R3, R4, R10, R15 and the new R16 (contest actions: `POST /api/contests/:id/resolve` and `…/withdraw`, no new permission, error code or audit action); RFC-66 R2; RFC-70 Context, R1–R4, R9, R10 (its Open question is settled); RFC-71 R3, R4; RFC-74 R3; RFC-33 R5. Add RFC-65 R16 to the new-rule table and to the final "every new rule number exists" check. Where task bodies assume a contest responds to one record's level, the amended rules win. The spec and plan edits are this ruling's exception to the Global Constraint that 13a edits only docs/rfc.
