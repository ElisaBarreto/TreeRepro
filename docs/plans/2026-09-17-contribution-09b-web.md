...
- `RecordDrawer`: badge `contests …` / `complements …` with a link to `respondsTo.id` (opens it in the same drawer via `onOpenRecord`); a "Responses" section listing `responses` (intent badge, author, date, open link); annotation rows show "supported by *citation*" when `reference` and a small `generated` badge "automatic".
