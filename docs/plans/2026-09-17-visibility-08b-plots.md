...
- `speciesVisible(v, activeCol?, idCol?)` adds `and exists (select 1 from plot_species ps where ps.species_id = <idCol> and ps.plot_id = any(${sql.param(v.plotIds)}::uuid[]))` when `v.plotIds !== null` — a JS array spliced straight into the `sql` tag becomes a list of SQL chunks, not a parameter (`docs/gotchas/dataset.md`); `sql.param(array)` (or `inArray`) binds it as one `uuid[]`. `plotIds = []` yields `sql\`false\`` (fail closed). Every existing caller passes `species.id` by default; the raw-SQL readers alias `species` and pass the aliased column: `summary.ts` (`sql\`s.id\`` after adding `join species s on s.id = r.species_id` to its three queries), `queues.ts` (`pendingTraits` and `listDisputed` join `species s`; `pendingGroups` joins `species s` on the pending rows), `curation.ts` `annotateRecord` (its record read joins `species`). Plot-bound viewers are rare (contributors bound by the admin), so the extra `exists` costs nothing for everyone else.
- Test helpers:
  ```ts
...
- `searchSpecies(db, visibility, input)` gains `scope?: 'plots' | 'all'`, `plotId?: string`, `viewerPlotIds?: string[]` (default `[]`: the viewer's assigned plots regardless of restriction, from `userScope`; optional so every existing and later test call stays valid) — the route computes `viewerPlotIds`; the service, in this order: `plotId` given → the plot exists (else 404 `PLOT_NOT_FOUND`) → the viewer may see it (`visibility.plotIds === null || visibility.plotIds.includes(plotId)`, else 403 `PERMISSION_DENIED`) → `exists plot_species for that plot`; no `plotId` → `scope ?? (viewerPlotIds.length > 0 && !visibility.inactive ? 'plots' : 'all')` (RFC-33 R6: managers and admins default to `all`); `plots` → `exists plot_species with plot_id = any(${sql.param(viewerPlotIds)}::uuid[])`; `all` with `visibility.plotIds !== null` → 403.
...
- [ ] **Step 2: Implement** — `plots`: `insert into plots (code, name, description, latitude, longitude, country, biome) select … where not exists (lower(code) = lower(s.plot_id))`, with `latitude`/`longitude` parsed by `case when x ~ '^[+-]?[0-9]+(\.[0-9]+)?$' then x::double precision end` and range-checked in the outcome column. `plot_species`: join `plots` on `lower(code)`, `species` on normalised name. `user_plots`: the e-mail hash cannot be computed in SQL, so `apply` does it in two steps inside the transaction — `select distinct user_email from import_staging` (the file is staged as-is, so the batch keeps the real file name and SHA-256), `getPii().blindIndex(email)` per distinct e-mail in Node, then `update import_staging s set email_hash = v.hash from (values …) as v(email, hash) where s.user_email = v.email` built with `sql.join` over at most 1,000 pairs per statement (chunk the list); then join `users.email_hash`. `raw_row` stores only `user_email` and `plot_id` (no hash; `jsonb_build_object` with the two columns). Document in RFC-68 R11 ("e-mails are matched through the blind index, computed by the command inside the import transaction").
...
- [ ] **Step 2:** README commands for the three imports (with the file-placement note) and the Admin › Plots page in the Layout paragraph; `docs/gotchas/import.md`: "user_plots stages the file untouched and hashes the distinct e-mails in Node inside the transaction (`update … from (values …)`); never write a temporary copy of a file with PII".
