 ...
 * copy: the same three numbers `WorkspacePage` reads off `dashboard.dataset`
 * (RFC-72 R1).
 */
 ...
 * RFC-72 R3 with the dataset's live counts substituted in. The copy lives
 * here, on its own, so the project owner can edit it without touching a
 * component; the contact e-mail is rendered as a link by the caller, not by
 * this module, which carries only the words.
 * @rfc RFC-72 R3
 */
export function projectDescription(counts: ProjectCounts): string {
  return (
    ...
    `${counts.referenceCount} references and ${counts.recordCount} records over ` +
    `${counts.speciesCount} species. ` +
