// `pnpm import:species-status --file <csv> [--run-by <email>]` — sets
// species.active (RFC-68 R8). Exit codes: 0 completed, 1 refused or failed, 2 usage.
...
import { importSpeciesStatus } from '../dataset/imports/species-status.ts';
import { createDb } from '../db/client.ts';
import { configurePii } from '../security/pii.ts';

const USAGE = 'usage: import-species-status --file <csv> [--run-by <email>]\n';
...
try {
  ...
  const batch = await importSpeciesStatus(db, { filePath: values.file, runBy });
