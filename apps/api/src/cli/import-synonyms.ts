// `pnpm import:synonyms --file <csv> [--run-by <email>]` — imports synonyms
// and common names (RFC-68 R12). Exit codes: 0 completed, 1 refused or
// failed, 2 usage.
...
import { importSynonyms } from '../dataset/imports/synonyms.ts';
import { createDb } from '../db/client.ts';
import { configurePii } from '../security/pii.ts';

const USAGE = 'usage: import-synonyms --file <csv> [--run-by <email>]\n';
...
try {
  ...
  const batch = await importSynonyms(db, { filePath: values.file, runBy });
