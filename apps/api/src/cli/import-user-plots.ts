// `pnpm import:user-plots --file <csv> [--run-by <email>]` — imports user plot assignments
// (RFC-68 R11). Exit codes: 0 completed, 1 refused or failed, 2 usage.
...
import { importUserPlots } from '../dataset/imports/plots.ts';
import { createDb } from '../db/client.ts';
import { configurePii } from '../security/pii.ts';

const USAGE = 'usage: import-user-plots --file <csv> [--run-by <email>]\n';
...
try {
  ...
  const batch = await importUserPlots(db, { filePath: values.file, runBy });
