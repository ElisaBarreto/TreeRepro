// `pnpm import:plot-species --file <csv> [--run-by <email>]` — imports plot species associations
// (RFC-68 R10). Exit codes: 0 completed, 1 refused or failed, 2 usage.
...
import { importPlotSpecies } from '../dataset/imports/plots.ts';
import { createDb } from '../db/client.ts';
import { configurePii } from '../security/pii.ts';

const USAGE = 'usage: import-plot-species --file <csv> [--run-by <email>]\n';
...
try {
  ...
  const batch = await importPlotSpecies(db, { filePath: values.file, runBy });
