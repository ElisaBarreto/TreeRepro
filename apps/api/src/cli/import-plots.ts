// `pnpm import:plots --file <csv> [--run-by <email>]` — imports field plots
// (RFC-68 R9). Exit codes: 0 completed, 1 refused or failed, 2 usage.
...
import { importPlots } from '../dataset/imports/plots.ts';
import { createDb } from '../db/client.ts';
import { configurePii } from '../security/pii.ts';

const USAGE = 'usage: import-plots --file <csv> [--run-by <email>]\n';
...
try {
  ...
  const batch = await importPlots(db, { filePath: values.file, runBy });
