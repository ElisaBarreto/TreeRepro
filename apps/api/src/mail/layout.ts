...
/** The amber pill; `href` and `label` are escaped here. @rfc RFC-10 R16 */
export function mailButton(href: string, label: string): string {
  return `<table role="presentation" cellpadding="0" cellspacing="0" border="0"><tr><td style="border-radius:999px;background-color:${MAIL_COLORS.pollen500};background-image:linear-gradient(90deg,${MAIL_COLORS.pollen500},${MAIL_COLORS.pollen400});"><a href="${escapeHtml(href)}" style="display:inline-block;padding:16px 30px;font-family:${BODY_FONT};font-size:16px;font-weight:700;line-height:20px;color:${MAIL_COLORS.ink};text-decoration:none;border-radius:999px;">${escapeHtml(label)} &rarr;</a></td></tr></table>`;
}
...
/** A tinted note box: `tone` picks canopy mist or amber. @rfc RFC-10 R16 */
export function mailNote(html: string, tone: 'mist' | 'amber' = 'mist'): string {
  const [bg, fg] =
    tone === 'amber'
      ? [MAIL_COLORS.amberTint, MAIL_COLORS.amberText]
      : [MAIL_COLORS.mist50, MAIL_COLORS.canopy800];
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 20px;"><tr><td style="padding:14px 16px;border-radius:12px;background-color:${bg};font-family:${BODY_FONT};font-size:14px;line-height:21px;color:${fg};">${html}</td></tr></table>`;
}
...
export function emailLayout(input: {
  ...
  const emblem = `${input.appOrigin}/email-emblem.png`;
