...
test.describe('RFC-60 R6 / RFC-69 species trait filters and breadcrumb (plan 10a)', () => {
  test('a trait filter in missing mode lists the species with no data, opening one names it in the breadcrumb, and clearing the category drops the filter', async ({
    ...
    // One shared prefix so the name box narrows the list to exactly these two
    // species, whatever else the stack holds by the time this file runs.
    ...
    trait = await create<TraitRow>(admin, `/api/traits/${trait.id}/levels`, { key: 'e2e_present' });
    ...
    // The deep link the trait page and the dashboard send people to. The name
    // is typed into the form rather than carried here, so nothing depends on
    // how a hand-written query string encodes a space.
    ...
    try {
      ...
      await expect(page.getByLabel('Category', { exact: true })).toHaveValue(CATEGORY_KEY);
